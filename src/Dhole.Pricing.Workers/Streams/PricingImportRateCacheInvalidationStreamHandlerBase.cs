...
internal abstract class PricingImportRateCacheInvalidationStreamHandlerBase(
...
) : IRedisStreamMessageHandler
{
    ...
    )
    {
        try
        {
            ...
            var importBatchId = TryGetGuid(document.RootElement, "importBatchId", "batchId");

            if (!importRateId.HasValue)
            {
                throw new InvalidOperationException(
                    $"El evento '{envelope.MessageType}' no contiene "
                        + "un identificador de tarifa importada válido."
                );
            }
            ...
            logger.LogInformation(
                "Pricing import rate cache invalidated. "
                    + "MessageType: {MessageType}, "
                    + "MessageId: {MessageId}, "
                    + "ImportRateId: {ImportRateId}, "
        ...
        catch (Exception exception)
        {
            logger.LogError(
                exception,
                "Failed to invalidate pricing import rate cache. "
                    + "MessageType: {MessageType}, "
                    + "MessageId: {MessageId}.",
                envelope.MessageType,
                envelope.MessageId
