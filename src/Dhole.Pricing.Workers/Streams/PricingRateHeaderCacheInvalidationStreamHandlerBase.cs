...
internal abstract class PricingRateHeaderCacheInvalidationStreamHandlerBase(
...
) : IRedisStreamMessageHandler
{
    ...
    )
    {
        try
        {
            ...
            if (!rateHeaderId.HasValue)
            {
                throw new InvalidOperationException(
                    $"El evento '{envelope.MessageType}' no contiene "
                        + "un identificador de tarifa válido."
                );
            }

            await cache.RemoveRateHeaderCacheAsync(rateHeaderId.Value, cancellationToken);

            logger.LogInformation(
                "Pricing rate header cache invalidated. "
        ...
        catch (Exception exception)
        {
            logger.LogError(
                exception,
                "Failed to invalidate pricing rate header cache. "
                    + "MessageType: {MessageType}, "
                    + "MessageId: {MessageId}.",
                envelope.MessageType,
                envelope.MessageId
